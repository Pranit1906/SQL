...
A) Conceptual design is a documentation technique. Once the relation schemes are defined one can draw E-R diagrams 
from the relation schemes for documentation
...
If a=b,then b=a.
A) if b=b,then a=a.
B) if b does not =a,then a does not=b.
C) if b=a,then b=a.
...
D) If it is hot,then it is summer.
...
B) DELETE from user WHERE Username = 'user1'
...
D) The base of a decimal number system is 10.
   ...
   42) By using …………………….., you can force immediate termination of the loop, bypassing the conditional expression and any remaining code in the body of the loop.
   A) switch
   B) break
   C) continue
   D) default
   ...
   C) If it is not round,then it can not roll.
   ...
   A) UnaryB) BinaryC) Both A and BD) None of the above
